...
export const getOrCreateDefaultSettings = async (saleorApiUrl: string, defaults: {
  ...
  const [created] = await db
    ...
    .values({
      ...
      nowpaymentsEnabled: defaults.nowpaymentsEnabled,
      moonpayEnabled: defaults.moonpayEnabled,
      rampnetworkEnabled: defaults.rampnetworkEnabled,
    })
    .returning();
  ...
  const [updated] = await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        nowpaymentsEnabled: input.nowpaymentsEnabled,
        moonpayEnabled: input.moonpayEnabled,
        rampnetworkEnabled: input.rampnetworkEnabled,
        updatedAt: new Date(),
      },
    })
    .returning();
...
export const searchPaymentSessions = async (saleorApiUrl: string, search: string) => {
...
export const getRecentPaymentSessions = async (saleorApiUrl: string, limit = 10) => {
  const rows = await db.query.paymentSessions.findMany({
